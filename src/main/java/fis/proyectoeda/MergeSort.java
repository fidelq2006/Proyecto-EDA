...
public class MergeSort {
    ...
    static void mergesortRatio(int izquierda,int derecha, SteamDatos[] array){
        if(izquierda>=derecha){
            return;
    }
        int mid=(derecha+izquierda)/2;
    ...
    static void mergeRatio(SteamDatos[]array,int izquierda,int mid,int derecha){
        SteamDatos[] temp= new SteamDatos[derecha-izquierda+1];
        int i=izquierda;
        int j=mid+1;
        int k=0;
    while(i<=mid&&j<=derecha){
     if(array[i].getRatioAprobacion() >=array[j].getRatioAprobacion()){
        temp[k]=array[i];
        i++;
    }else {
            temp[k]=array[j];
